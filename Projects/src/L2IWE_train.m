...
%     sigma_list=xscale*[1/10 1/5 1/2 2/3 1 1.5 2 5 10]; % Candidates of Gaussian width
      ...
      for gamma_index=1:length(gamma_list)
        ...
        for lambda_index=1:length(lambda_list)
          ...
          for k=1:fold
            ...
            alpha_cv=linsolve(Kcvtr'*Kcv_w+lambda*eye(b),Kcv_w'*ycvtr');
            score(k)=mean(((ycvte'-Kcvte*alpha_cv).^2).*w(cv_split==k)');
          end % for fold
          score_cv(gamma_index,lambda_index)=mean(score);
  ...
  model.alpha=linsolve(K'*K_w+lambda_chosen*eye(b),K_w'*y');
